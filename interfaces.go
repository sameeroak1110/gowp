...
// - The upstream is assumed to pass context to the downstream - ie, Process() method - so that
// each running job (Process() method in execution) can terminate gracefully to avoid go-routine leak.
// - This's essential as a specific business requirment may need each specific running job to finish
// in entirety and gracefully and thus conclude logically. For instance, a job is updating some rows in
// a table and needs to know whether the update was successful or not.
// - However, there may be a circumstance where the business need is to terminate the running job instantly
// when it receives the cancelled context.
